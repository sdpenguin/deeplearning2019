...
def eval_verification(tskdir,descr,split):
...
def gen_verif(tskdir,seqs,split,N_pos=1e6,N_neg=1e6):
    np.random.seed(42)
...
# Matching task #
#################
def eval_matching(tskdir,descr,split):
...
def eval_retrieval(tskdir,descr,split): #WIP
...
def gen_retrieval(tskdir,seqs,split,N_queries=0.5*1e4,N_distractors=2*1e4):
    np.random.seed(42)
    seq2len = seqs_lengths(seqs)
